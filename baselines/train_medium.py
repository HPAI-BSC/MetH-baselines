...
dataset_paths = Paths.MetHMedium

current_date = '{date:%Y-%m-%d_%H-%M-%S}'.format(date=datetime.datetime.now())
model_path = os.path.join(PROJECT_PATH, 'model_checkpoints', 'medium_{}.ckpt'.format(current_date))
summaries_path = os.path.join(PROJECT_PATH, 'summaries', 'medium_{}'.format(current_date))
